...
# select a language: need to select a visible one or add another interaction
# to get visible all languages
def filter_lang(driver, language):
    # another option: define XPath
    driver.find_element_by_xpath('//div[@class=\'ui_radio item\' and @data-value=\'{}\']'.format(language)).click()
...
for url in urls:
    ...
    # filter language of reviews to scrape
    filter_lang(driver, 'en')
    ...
    while n_reviews < N_MAX:
        ...
        # apply the pipeline to collect enough reviews
        driver.get(url_)
        driver.execute_script('window.scrollBy(0,1800)')
        expand_review(driver)
        response = BeautifulSoup(driver.page_source, 'html.parser')
